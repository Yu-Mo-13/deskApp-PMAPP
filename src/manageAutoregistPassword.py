...
# ユーザー一覧を取得
curl = Curl(get_config("CURLURL", "ROOTURL") + get_config("CURLURL", "AUTOREGISTURL"))

try:
    autoregist_list = curl.get()
...
for autoregist in autoregist_list:
    row += 1
    layout.append([sg.InputText(size=list_size, font=font, key="app" + str(row), default_text=autoregist["app"], disabled=True),
                   sg.InputText(size=list_size, font=font, key="other_info" + str(row), default_text=autoregist["other_info"], disabled=True),
                   sg.InputText(size=list_size, font=font, key="pwd" + str(row), default_text=encryption.decrypt(autoregist["pwd"]), disabled=True, password_char="*"),
...
while True:
    ...
    if event.startswith("regist"):
        confirm_regist = sg.PopupYesNo("パスワードを本登録しますか。", font=font_popup, title=get_config("MODULECONSTANT", "TITLE"))
        # eventから登録ボタンの番号を取得
