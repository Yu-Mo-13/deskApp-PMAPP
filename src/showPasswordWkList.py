...
for row in range(5):
    layout.append(
        [
            sg.Text("パスワード" + str(row + 1), font=font),
            sg.InputText(
                size=size, font=font, key="password" + str(row + 1), disabled=True
            ),
            sg.Text("アプリ名" + str(row + 1), font=font),
            sg.InputText(
                size=size, font=font, key="application" + str(row + 1), disabled=True
            ),
            sg.Text("アカウント" + str(row + 1), font=font),
            sg.InputText(
                size=size, font=font, key="other_info" + str(row + 1), disabled=True
            ),
...
layout.append(
    [sg.Button("取得", font=font, key="get"), sg.Button("終了", font=font, key="quit")]
)
...
while True:
    ...
    if event.startswith("regist"):
        ...
        if not (pwd) or not (app):
            ...
            confirm_regist = sg.PopupYesNo(
                "パスワードを登録しますか。", font=font_popup, title=title_popup_success
            )
    ...
    # 終了ボタン
    if event == "quit":
        message_quit = sg.PopupYesNo(
            "この画面を閉じますか。", font=font_popup, title=title_popup_success
        )
        if message_quit == "Yes":
            break
