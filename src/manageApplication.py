...
while True:
    ...
    if event == "regist":
        ...
        if confirm_regist == "Yes":
            ...
            if app_name == "":
                ...
                sg.PopupOK(
                    "アプリ名が入力されていません。", font=font_popup, title=title_popup
                )
                ...
                try:
                    if application_detail["no"] == 0:
                        # 登録処理
                        application.regist(
                            app_name, account_class, notice_class, mark_class, auto_size
                        )
    ...
    if event == "cancel":
        sg.PopupOK(
            "アプリケーションを終了します。", font=font_popup, title=title_popup_success
        )
        break
